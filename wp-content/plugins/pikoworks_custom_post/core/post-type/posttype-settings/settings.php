...
{
    wp_die( esc_html__('You do not have sufficient permissions to access this page.', 'pikoworks_custom_post') );
}
...
if($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST[ $post_type.'-slug' ])){
    ...
    if($_POST[ $post_type.'-slug' ]!='' || $_POST['category-'. $post_type.'-slug' ]!=''){
        ...
        if(!$is_match_slug || !$is_match_taxonomy_slug){
            update_option('piko-'.$post_type.'-config',$config);
...
<form name="frmChangePostTypeSlug" method="post" action="">
    <div class="page-post-type-setting">
        <h3><?php esc_html_e('Custom Post Change Slug', 'pikoworks_custom_post'); ?></h3>
        <ul>
            <li class="title">Current slug post type : </li>
            <li class="value"><?php echo esc_attr($post_type_slug) ?> &nbsp;</li>
            <li class="title"><?php esc_html_e('New slug post type:', 'pikoworks_custom_post'); ?> </li>
            <li class="value"><input type="text" name="<?php echo esc_attr($post_type.'-slug') ?>"/></li>

            <li class="title"> <?php esc_html_e('Current name post type :', 'pikoworks_custom_post'); ?></li>
            <li class="value"><?php echo esc_attr($post_type_name) ?> &nbsp;</li>
            <li class="title"><?php esc_html_e('New name post type :', 'pikoworks_custom_post'); ?></li>
            <li class="value"><input type="text" name="<?php echo esc_attr($post_type.'-name') ?>"/></li>

            <?php if($taxonomy_slug!=''){ ?>
                <li class="title"><?php esc_html_e('Current slug post type :', 'pikoworks_custom_post'); ?></li>
                <li class="value"><?php echo esc_attr($taxonomy_slug) ?> &nbsp;</li>
                <li class="title"><?php esc_html_e('New slug post type: ', 'pikoworks_custom_post'); ?></li>
                <li class="value"><input type="text" name="<?php echo esc_attr('category-'.esc_attr($post_type).'-slug') ?>"/></li>

                <li class="title"><?php esc_html_e('Current name category :', 'pikoworks_custom_post'); ?></li>
                <li class="value"><?php echo esc_attr($taxonomy_name) ?> &nbsp;</li>
                <li class="title"><?php esc_html_e('New name category :', 'pikoworks_custom_post'); ?> </li>
                <li class="value"><input type="text" name="<?php echo esc_attr('category-'.esc_attr($post_type).'-name') ?>"/></li>
            <?php } ?>

            <li class="title"></li>
            <li class="value"><input class="button button-large button-primary" type="submit" value="Save Changes"/></li>
        </ul>
        <i><?php esc_html_e('NB: After Changing the post slug Then Refresh Admin Panel.', 'pikoworks_custom_post');?> </i>
    </div>
</form>
