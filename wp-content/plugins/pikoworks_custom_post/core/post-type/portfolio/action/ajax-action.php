...
add_action("wp_ajax_nopriv_pikowroks_portfolio_search", "pikowroks_portfolio_search");
add_action("wp_ajax_pikowroks_portfolio_search", "pikowroks_portfolio_search");

function pikowroks_portfolio_search(){

    function pikoworks_portfolio_search_title_filter( $where, &$wp_query ) {
/**
 * action search
 */
        global $wpdb;
    ...
    if ( $keyword ) {
        ...
        add_filter( 'posts_where', 'pikoworks_portfolio_search_title_filter', 10, 2 );
        $search = new WP_Query( $search_query );
        remove_filter( 'posts_where', 'pikoworks_portfolio_search_title_filter', 10, 2 );
        ...
        if ($search && isset($search->post) && count($search->post) > 0) {
            ...
            $new_data[] = array(
                'id'        => -1,
                'title'     => esc_html__('Sorry, but nothing matched your search terms. Please try again with different keywords.','pikoworks_custom_post'),
                'date'      => null
            );
        }
        wp_reset_postdata();
