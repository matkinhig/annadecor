...
function pikoworks_custom_css_in_widget_form( $widget, $return, $instance ) {
	if ( !isset( $instance['css_class'] ) ) $instance['css_class'] = null;

	echo "<p><label for='widget-{$widget->id_base}-{$widget->number}-css_class'>". esc_html__( 'CSS Class', 'pikoworks_core' ) .":</label>
...
// Update widget callback function
function pikoworks_custom_css_widget_update_callback( $instance, $new_instance ) {
	$instance['css_class'] = $new_instance['css_class'];
	$instance['ids']     = $new_instance['ids'];
	return $instance;
}

// Add Custom CSS Class to front end function
function pikoworks_custom_css_add_widget_css_class( $params ) {
	...
	// if Widget Logic plugin is enabled, use it's callback function
	if ( in_array( 'widget-logic/widget_logic.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) {
		...
		// if Widget Context plugin is enabled, use it's callback function
...
// Front end hook function
function pikoworks_custom_css_frontend_hook() {
	if ( !is_admin() ) {
		add_filter( 'dynamic_sidebar_params','pikoworks_custom_css_add_widget_css_class' );
	}
}

add_action( 'in_widget_form', 'pikoworks_custom_css_in_widget_form', 10, 3 );
add_filter( 'widget_update_callback', 'pikoworks_custom_css_widget_update_callback', 10, 2 );
add_action( 'wp_loaded', 'pikoworks_custom_css_frontend_hook' );