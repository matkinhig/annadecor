<?php
/**
 * Widget API: newsletter
 *
 * @author themepiko
 */

/**
 * Core class used to implement a Custom newsletter widget.
 *
 * @see WP_Widget
 */
class pikoworks_widget_newsletter extends WP_Widget {
	...
	public function __construct() {
		$widget_ops = array(
			'classname' => 'widget_piko_newsletter',
			'description' => esc_html__( 'Display Newsletter from code.' ),
		...
		parent::__construct( 'custom_newsletter', esc_html__( '[Pikoworks] Newsletter' ), $widget_ops, $control_ops );
	}
	...
	public function widget( $args, $instance ) {
		...
		if ( ! empty( $title ) ) {
			$title = '<h5 class="fs36">'.esc_attr($title).'</h5>';
		}
		if ( ! empty( $sub_title ) ) {
			$sub_title = '<div class="title">'.esc_attr($sub_title).'</div>';
		}
		echo '<div class="piko-newsletter layout-'.esc_attr($layout).'"><div class="row align-items-center">'; // The textwidget class is for theme styling compatibility.
		echo '<div class="col-12 col-md-7">'.balanceTags($title . $sub_title).'</div>';
		echo '<div class="col-12 col-md-5 search-form">'.$content.'</div>';
		echo '</div></div>';
	...
	public function form( $instance ) {
                $defaults = array( 'title' => esc_html__('Sign up our Newsletter', 'pikoworks_core'),'sub_title' => esc_html__('Receive 10% off for first three Order', 'pikoworks_core'), 'content' => '' );            
		$instance = wp_parse_args( (array) $instance, $defaults );
		?>
                <p>
                       <label for="<?php echo $this->get_field_id( 'sub_title' ); ?>"><?php esc_html_e('Sub Title:', 'pikoworks_core') ?></label>
                       <input id="<?php echo $this->get_field_id( 'sub_title' ); ?>" name="<?php echo $this->get_field_name( 'sub_title' ); ?>" value="<?php echo $instance['sub_title']; ?>" style="width:100%;" />
                </p>
		...
		<p><label for="<?php echo esc_attr( $this->get_field_id( 'layout' ) ); ?>"><?php esc_html_e( 'Design Layout', 'pikoworks_core' ); ?>:</label>
			<select id="<?php echo esc_attr( $this->get_field_id( 'layout' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'layout' ) ); ?>" class="widefat">
				<option value="1" <?php selected( '_self' ); ?>><?php esc_html_e( 'Layout 1', 'pikoworks_core' ); ?></option>
				<option value="2" <?php selected( '_blank' ); ?>><?php esc_html_e( 'Layout 2', 'pikoworks_core' ); ?></option>
			</select>
		</p>
